"""
Expense flow — fully inline-based FSM.
"""
import logging
...
from services.finance_service import add_expense, register_user
...
async def start_add_expense(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        "💸 *Xarajat qo'shish — 1/3*\n\n"
        "Miqdorni kiriting (masalan: `15000`):",
        parse_mode="Markdown",
...
@router.callback_query(
    AddExpenseStates.waiting_category,
    F.data.startswith("cat_expense:"),
)
...
@router.callback_query(
    AddExpenseStates.waiting_description,
    F.data == "desc:skip",
)
async def expense_desc_skip(callback: CallbackQuery, state: FSMContext) -> None:
    await _save_expense(callback.message, state, callback.from_user, description=None, edit=True)
    await callback.answer()
...
async def expense_description(message: Message, state: FSMContext) -> None:
    await _save_expense(message, state, message.from_user, description=message.text, edit=False)


# ─── SAVE ─────────────────────────────────────────────────────────────────────

async def _save_expense(msg_obj, state, from_user, description, edit: bool) -> None:
    data = await state.get_data()
    async with AsyncSessionLocal() as session:
        user = await register_user(session, from_user.id, from_user.username, from_user.full_name)
        tx = await add_expense(
            session,
            user_id=user.id,
            amount=data["amount"],
